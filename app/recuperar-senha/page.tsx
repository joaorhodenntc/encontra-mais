...
export default function ResetPasswordPage() {
  ...
  const onSubmit = async (data: ResetPasswordFormValues) => {
    ...
    try {
      ...
      setFormError(error.message || "Ocorreu um erro ao processar sua solicitação. Tente novamente.")
    } finally {
      setIsLoading(false)
    }
  }
    ...
    return (
      <Card className="mx-auto w-full max-w-md">
        <CardHeader className="space-y-1">
          ...
          <CardDescription>Verifique seu email para redefinir sua senha</CardDescription>
        </CardHeader>
        <CardContent>
          <Alert className="mb-6 bg-green-50">
            <CheckCircle2 className="h-4 w-4 text-green-600" />
            <AlertTitle className="text-green-600">Solicitação enviada com sucesso</AlertTitle>
            <AlertDescription>
              Enviamos um email para <strong>{resetEmail}</strong> com instruções para redefinir sua senha. Por favor,
          ...
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              O link de recuperação de senha é válido por 24 horas. Se você não receber o email, verifique sua pasta de
